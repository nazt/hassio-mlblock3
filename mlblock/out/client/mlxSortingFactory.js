...
class mlxSortingFactory extends mlxElementLocakableOutput {
  ...
  preload() {
    ...
    this.img_marker_red = this.p5.loadImage('assets_game/mlxSortingFactory/p_marker_red.png')
    this.img_marker = this.p5.loadImage('assets_game/mlxSortingFactory/p_marker_black.png')
    this.img_mask = this.p5.loadImage('assets_game/mlxSortingFactory/mask.png')
    this.img_step1 = this.p5.loadImage('assets_game/mlxSortingFactory/step2.png')
    this.img_step2 = this.p5.loadImage('assets_game/mlxSortingFactory/step3.png')
    this.img_building_left = this.p5.loadImage('assets_game/mlxSortingFactory/factory_left.png')
    this.img_building_right = this.p5.loadImage('assets_game/mlxSortingFactory/factory_right.png')
    this.img_heart = this.p5.loadImage('assets_game/mlxSortingFactory/heart.png')
    this.img_box_blue = this.p5.loadImage('assets_game/mlxSortingFactory/box_blue.png')
    this.img_box_red = this.p5.loadImage('assets_game/mlxSortingFactory/box_red.png')
    this.img_box_yellow = this.p5.loadImage('assets_game/mlxSortingFactory/box_yellow.png')
    this.img_right = this.p5.loadImage('assets_game/mlxSortingFactory/box_right.png')
    this.img_wrong = this.p5.loadImage('assets_game/mlxSortingFactory/box_wrong.png')
    this.img_heart = this.p5.loadImage('assets_game/mlxSortingFactory/heart.png')
    this.img_heart_empty = this.p5.loadImage('assets_game/mlxSortingFactory/heart_empty.png')
    this.img_waiting = this.p5.loadImage('assets_game/mlxSortingFactory/waittoconnect.png')
    this.font = this.p5.loadFont('assets_game/mlxSortingFactory/Anakotmai-Bold.ttf')
    this.sound_beep = this.p5.loadSound('assets_game/mlxSortingFactory/beep.mp3')
    this.sound_changepoint = this.p5.loadSound('assets_game/mlxSortingFactory/beep.mp3')
    this.sound_ok = this.p5.loadSound('assets_game/mlxSortingFactory/ok.mp3')
    this.sound_falldown = this.p5.loadSound('assets_game/mlxSortingFactory/falldown.mp3')
    this.sound_correct = this.p5.loadSound('assets_game/mlxSortingFactory/correct.mp3')
    this.sound_movebridge = this.p5.loadSound('assets_game/mlxSortingFactory/movebridge.mp3')
    this.sound_wrong = this.p5.loadSound('assets_game/mlxSortingFactory/wrong.mp3')
    this.sound_over = this.p5.loadSound('assets_game/mlxSortingFactory/gameover.mp3')
    this.sound_start = this.p5.loadSound('assets_game/mlxSortingFactory/start.mp3')
  ...
  draw(p5) {
    ...
    if (this.inElement != null && this.inElement.inElement != null && this.inElement.inElement.videoCapture != null) {
      ...
      this.precanvas.image(this.inElement.inElement.videoCapture, 0, 0, this.canvas_w, this.canvas_h)
      this.precanvas.pop()
  ...
  distance(pos1, pos2) {
    var dist = this.p5.sqrt(this.p5.pow(pos2.x - pos1.x, 2) + this.p5.pow(pos2.y - pos1.y, 2))
    return dist
  }
  ...
  checkRaiseHand() {
    ...
    if (
      ...
      if (
        this.distance(this.posLeftElbow[0], this.prevLeftElbowPos) <= this.ALLOWED_CALIBRATE_MOVING_PIXEL &&
        this.distance(this.posRightElbow[0], this.prevRightElbowPos) <= this.ALLOWED_CALIBRATE_MOVING_PIXEL
      ) {
        return true
      }
    }
...
class SortingFactoryBox {
  constructor(platform, target, mlxParent) {
    ...
    this.imageList = [this.parent.img_box_yellow, this.parent.img_box_red, this.parent.img_box_blue]
  ...
  draw() {
    if (!this.delete) {
      this.x += this.parent.p5.floor(this.parent.SPEED * (this.parent.p5.deltaTime / 1000))
      if (this.x > this.parent.canvas_w + this.parent.BOX_WH / 2) this.delete = true
      ...
      if (!this.drop) {
        if (this.x < this.xPos[0]) {
          //on origin platform
          this.parent.precanvas.translate(this.x, this.yPos[this.originPlatform])
          ...
          if (!this.checked && this.x < this.parent.canvas_w - 70 && this.targetPlatform == this.originPlatform) {
            //correct platform
            this.correct = 1
      ...
      if (this.drop) {
        ...
        if (this.y > this.parent.canvas_h + this.parent.BOX_WH / 2) this.delete = true
      }
  ...
  calculateHeight() {
    ...
    var yCurrent =
      ...
      percent * (this.yPos[this.parent.ui_game.selectedPointR] - this.yPos[this.parent.ui_game.selectedPointL])
    return yCurrent
  }
}
//===============================
...
class SortingFactory_UI_Game {
  ...
  draw() {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      if (i < this.parent.HP) this.parent.precanvas.image(this.parent.img_heart, this.parent.canvas_w - 30 - 54 * i, 28)
      else this.parent.precanvas.image(this.parent.img_heart_empty, this.parent.canvas_w - 30 - 54 * i, 28)
    }
    this.parent.precanvas.pop()
      ...
      if (this.releasebox < 0) {
        ...
        if (this.parent.boxes.length == 0) {
          ...
          if (choice == 1) {
            ...
            this.releasebox = this.parent.RELESEBOX_CONDITION_1 - 850 * ((this.parent.SPEED - 65) / 50)
            this.parent.boxes[this.parent.boxes.length] = this.nextBox
            this.nextBox = new SortingFactoryBox(
            ...
            this.releasebox = this.parent.RELESEBOX_CONDITION_2 - 1200 * ((this.parent.SPEED - 65) / 50)
            this.parent.boxes[this.parent.boxes.length] = this.nextBox
            var target = this.parent.p5.random([0, 1, 2])
            var platform = this.parent.p5.random([0, 1, 2])
            while (this.parent.boxes[this.parent.boxes.length - 1].originPlatform == platform) {
              target = this.parent.p5.random([0, 1, 2])
              platform = this.parent.p5.random([0, 1, 2])
            }
            this.nextBox = new SortingFactoryBox(platform, this.parent.p5.random([0, 1, 2]), this.parent)
          } else {
            //diff box , diff platform
            this.releasebox = this.parent.RELESEBOX_CONDITION_3 - 1500 * ((this.parent.SPEED - 65) / 50)
            this.parent.boxes[this.parent.boxes.length] = this.nextBox
            var target = this.parent.p5.random([0, 1, 2])
            var platform = this.parent.p5.random([0, 1, 2])
            while (
              this.parent.boxes[this.parent.boxes.length - 1].originPlatform == platform ||
              this.parent.boxes[this.parent.boxes.length - 1].targetPlatform == target
            ) {
              target = this.parent.p5.random([0, 1, 2])
              platform = this.parent.p5.random([0, 1, 2])
            }
  ...
  calculateCurrentSelectedPoint() {
    ...
    if (this.previousPointL != this.selectedPointL) this.parent.sound_movebridge.play()
    if (this.previousPointR != this.selectedPointR) this.parent.sound_movebridge.play()
    this.previousPointL = this.selectedPointL
    this.previousPointR = this.selectedPointR
  }

  getCurrentAngle() {
    if (this.degreeL == null) this.degreeL = this.parent.degree(this.parent.posNose[0], this.parent.posLeftElbow[0])
    if (this.degreeR == null) this.degreeR = this.parent.degree(this.parent.posNose[0], this.parent.posLeftElbow[0])
    this.degreeL = (this.degreeL + this.parent.degree(this.parent.posNose[0], this.parent.posLeftElbow[0])) / 2 //simple filter
    this.degreeR = (this.degreeR + this.parent.degree(this.parent.posNose[0], this.parent.posRightElbow[0])) / 2
  ...
  drawBackground() {
    ...
    this.parent.precanvas.rect(0, 0, this.parent.canvas_w, this.parent.precanvas.canvas_h)
    this.parent.precanvas.imageMode(this.parent.p5.CORNER)
    this.parent.precanvas.image(this.parent.img_building_left, 0, 43, 231, 516)
    this.parent.precanvas.image(this.parent.img_building_right, 630, 43, 223, 516)
    this.parent.precanvas.pop()
  }
  ...
  drawMarkerPoint() {
    ...
    for (var i = 0; i < 3; i++) {
      ...
      if (i == this.selectedPointL) {
        this.parent.precanvas.image(this.parent.img_marker_red, this.leftPos[i].x, this.leftPos[i].y)
      } else {
        this.parent.precanvas.image(this.parent.img_marker, this.leftPos[i].x, this.leftPos[i].y)
      }
      //right point
      if (i == this.selectedPointR) {
        this.parent.precanvas.image(this.parent.img_marker_red, this.rightPos[i].x, this.rightPos[i].y)
      } else {
        this.parent.precanvas.image(this.parent.img_marker, this.rightPos[i].x, this.rightPos[i].y)
      }
      this.parent.precanvas.pop()
    }
  }
  ...
  drawIntro() {
    this.parent.precanvas.push()
    this.parent.precanvas.imageMode(this.parent.p5.CORNER)
    this.parent.precanvas.image(this.parent.img_logo, 0, 0, this.parent.canvas_w, this.parent.canvas_h)
    this.parent.precanvas.pop()
    this.parent.precanvas.push()
    ...
    this.parent.precanvas.fill(
      ...
      170,
      0,
      this.parent.p5.abs(this.parent.p5.sin((this.parent.p5.TWO_PI / 50) * this.alpha_2)) * 255
    )
    this.parent.precanvas.textAlign(this.parent.p5.CENTER, this.parent.p5.CENTER)
    this.setTextSizeStrokeWeight(35, 0, 5)
    this.parent.precanvas.text('raise both hands to start !!', this.parent.canvas_w / 2, this.parent.canvas_h - 70)
    this.parent.precanvas.pop()
    if (this.parent.checkRaiseHand()) {
      ...
      if (this.checkingCountDown <= 2001) {
        ...
        this.parent.precanvas.rect(0, 0, this.parent.canvas_w, this.parent.vanvas_h)
        this.parent.precanvas.push()
        this.parent.precanvas.textAlign(this.parent.p5.CENTER, this.parent.p5.CENTER)
  ...
  drawOver() {
    ...
    this.parent.precanvas.fill(
      ...
      170,
      0,
      this.parent.p5.abs(this.parent.p5.sin((this.parent.p5.TWO_PI / 50.0) * this.alpha_2)) * 255
    )
    this.parent.precanvas.textAlign(this.parent.p5.CENTER, this.parent.p5.CENTER)
    this.setTextSizeStrokeWeight(35, 0, 5)
    this.parent.precanvas.text('raise both hands to restart!!', this.parent.canvas_w / 2, this.parent.canvas_h - 70)
    this.parent.precanvas.pop()
    if (this.parent.checkRaiseHand()) {
      this.checkingCountDown -= this.parent.p5.deltaTime
      this.beepSoundTime -= this.parent.p5.deltaTime
      if (this.checkingCountDown <= 2001) {
        this.parent.precanvas.push()
        this.parent.precanvas.fill(3, 143, 0, 60)
        this.parent.precanvas.rect(0, 0, this.parent.canvas_w, this.parent.canvas_h)
        this.parent.precanvas.textAlign(this.parent.p5.CENTER, this.parent.p5.CENTER)
        this.setTextSizeStrokeWeight(100, 0, 5)
        this.parent.precanvas.fill(255)
        this.parent.precanvas.text(
      ...
      this.parent.precanvas.text('GAME OVER', this.parent.canvas_w / 2, this.parent.canvas_h / 2)
      this.parent.precanvas.pop()
      this.checkingCountDown = 3000
      //store prev data
      this.parent.prevLeftElbowPos = this.parent.posLeftElbow[0]
...
class SortingFactory_UI_Calibrate {
  ...
  drawSetupText() {
    this.parent.precanvas.push()
    this.parent.precanvas.imageMode(this.parent.p5.CORNER)
    this.parent.precanvas.image(this.parent.img_mask, 0, 0, this.parent.canvas_w, this.parent.canvas_h)
    this.parent.precanvas.fill(255)
    this.parent.precanvas.rect(20, this.parent.canvas_h - 120, this.parent.canvas_w - 40, 100, 15)
    //----------------------------------
    // STEP 1
    // check if user raise hand
    if (this.calibrateStep == 'step1') {
      ...
      if (this.checkingCountDown >= 2001) {
        ...
        //start count down
        this.parent.precanvas.text(
          'Stay Still(' + (this.parent.p5.int(this.checkingCountDown / 1000) + 1) + ' s.)',
          this.parent.canvas_w / 2,
          this.parent.canvas_h - 70
        )
        this.parent.precanvas.fill(3, 143, 0, 60)
      ...
      this.parent.precanvas.image(this.parent.img_step1, 20, this.parent.canvas_h - 120 - 231, 263, 231)
      this.ratio_w = this.parent.canvas_w / this.parent.inElement.inElement.videoWidth
      this.ratio_h = this.parent.canvas_h / this.parent.inElement.inElement.videoHeight
      ...
      if (this.parent.checkRaiseHand()) {
        ...
        if (this.checkingCountDown < 0) {
          ...
          this.parent.MAX_L_ANGLE = this.parent.p5.int(
            this.parent.degree(this.parent.posNose[0], this.parent.posLeftElbow[0])
          )
          this.parent.MIN_R_ANGLE = this.parent.p5.int(
            this.parent.degree(this.parent.posNose[0], this.parent.posRightElbow[0])
          )
    ...
    //check for lower pos of elbow
      ...
      this.setTextSizeStrokeWeight(35, 0, 5)
      if (this.checkingCountDown >= 2001) {
        this.parent.precanvas.text('lower both elbows', this.parent.canvas_w / 2, this.parent.canvas_h - 70)
      } else {
        //start count down
        this.parent.precanvas.text(
          'Stay Still(' + (this.parent.p5.int(this.checkingCountDown / 1000) + 1) + ' s.)',
      ...
      this.parent.precanvas.image(this.parent.img_step2, 20, this.parent.canvas_h - 120 - 225, 228, 225)
      this.ratio_w = this.parent.canvas_w / this.parent.inElement.inElement.videoWidth
      this.ratio_h = this.parent.canvas_h / this.parent.inElement.inElement.videoHeight
      this.drawElbowMarker() //draw marker at elbow
