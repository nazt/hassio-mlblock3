...
class mlxFaceMaskDetector extends mlxElementModel {
  ...
  createElementMenu() {
    ...
    this.downloadMenu = menu.addCheckedMenu(
      "Download Model",
      this.flip,
      (ui, value) => {
...
class mlxCorgiDudeCamera2 extends mlxElementInput {
  constructor(mlx) {
    ...
    this.corgihub.register(
      "video",
      this.constructor.name,
      this.setVideo.bind(this)
    );
    setTimeout(() => {
  ...
  doCreateVideoCapture() {
    ...
    try {
      ...
      this.videoCapture = this.mlx.p5.createImage(
        this.videoRawWidth,
        this.videoRawHeight
      );
...
class mlxCorgiDudeMobileNet extends mlxImageClassifier {
  constructor(mlx) {
    ...
    this.model = ml5.imageClassifier(
      "http://localhost:8089/models/transfer_mb25_224/model.json",
      () => {
      ...
      if (!this.outElement || !this.outputIsReady) {
        ...
        ml5.tf.tidy(() => {
          ...
          if (img.shape[0] !== IMAGE_SIZE || img.shape[1] !== IMAGE_SIZE) {
            ...
            resized = ml5.tf.image.resizeBilinear(
              normalized,
              [224, 224],
              alignCorners
            );
...
class mlxCorgiDudeMobileNetClassifier extends mlxFeatureExtractor {
  constructor(mlx) {
    ...
    const options = {
      numLabels: 5,
      mobilenetURL:
        "http://localhost:8089/models/transfer_mb25_224/layers/model.json",
      graphModelURL:
        "http://localhost:8089/models/transfer_mb25_224/graph/model.json",
      embeddingLayer:
        "StatefulPartitionedCall/mobilenet_0.25_224/reshape_2/Reshape",
    };
    try {
          ...
          this.regressor.train((lossValue) => {
            if (lossValue) {
              ...
              this.labelTrain.setText(
                "Done Training! Final Loss: " + this.loss
              );
              this.trained = true;
              this.drawed = false;
            }
