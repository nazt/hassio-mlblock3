...
class mlxVideoCapture extends mlxElementInput {
  ...
  createElementMenu() {
    ...
    {
      this.cameraMenu = menu.addSelectMenu(
...
class mlxDrawCanvas extends mlxElementInput {
  constructor(mlx) {
    ...
    this.h =
      this.canvasSize + this.mlx.mlxCaptionHeight + this.mlx.mlxPadding * 2;
...
class mlxMouseInput extends mlxElementInput {
  constructor(mlx) {
    ...
    this.output = [];
    this.ready = true;
    this.backgroundMap = this.p5.createGraphics(
      this.canvasSize,
      this.canvasSize
    );
  ...
  getResults(results) {
    ...
    if (
      this.activeData &&
      results.data[0] === this.activeData.data[0] &&
      results.data[1] === this.activeData.data[1]
    ) {
    ...
    if (!this.isTraining && this.outElement && this.outElement.classify) {
      ...
      for (var y = 0; y < this.canvasSize; y += 10) {
        for (var x = 0; x < this.canvasSize; x += 10) {
          datas.push([
            x + step / 2 - this.canvasSize / 2,
            y + step / 2 - this.canvasSize / 2,
          ]);
        }
      }
...
class mlxImage extends mlxElementInput {
  ...
  createElementMenu() {
    ...
    this.autoResizeMenu = menu.addCheckedMenu(
      "Auto resize",
      this.autoResize,
      (ui, value) => {
        this.hideMenu();
  ...
  update() {
    ...
    if (this.divW == 0 && this.img && this.img.width && this.img.height) {
      ...
      this.div.position(
        this.x + this.contentX,
        this.y + this.contentY + this.mlx.mlxNavBarHeight
      );
    ...
    if (img) {
      ...
      if (!this.noDraw) {
        ...
        if (img.width / this.divW > img.height / this.divH) {
          ...
          p5.image(
            img,
            (this.x + this.contentX) / this.contentScale,
            (this.y + this.contentY) / this.contentScale
          );
        } else {
...
class mlxVideoPixels extends mlxElementInput {
  ...
  doProcess() {
    ...
    // coso
    console.log(
      this.videoCapture.width,
      this.videoCapture.height,
      this.videoCapture.pixels
    );
    // if (window.conn) window.conn.send(JSON.stringify(this.videoCapture.pixels));
    if (window.conn)
      window.conn.send(
        JSON.stringify({ pixels: Array.from(this.videoCapture.pixels) })
      );
    return true;
  }
  ...
  draw(p5) {
    ...
    if (this.ready) {
      ...
      if (
        this.videoCapture.width != this.videoSize ||
        this.videoCapture.height != this.videoSize
      ) {
        this.videoCapture.size(this.videoSize, this.videoSize);
      }
      ...
      for (let x = 0; x < this.videoCapture.width; x++) {
        for (let y = 0; y < this.videoCapture.height; y++) {
          ...
          this.p5.fill(r, g, b);
          this.p5.rect(
            this.x + this.contentX + x * w,
            this.y + this.contentY + y * w,
            w,
            w
          );
          //this.videoData.push(r / 255);
