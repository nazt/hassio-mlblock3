...
var mlxScaventureHuntEndSpeak = true;

class mlxScaventureHunt extends mlxElementOutput {
  constructor(mlx) {
    //Game Value
    super(mlx);
    this.title = "Scaventure Hunt";
    this.category = "Output";
    this.type = "ScaventureHunt";
    this.inType = "results";
    ...
    this.h =
      this.canvas_h + this.mlx.mlxCaptionHeight + this.mlx.mlxPadding * 2; //size including frame
    this.fps = 10;
  ...
  preload() {
    ...
    this.sound_blip = this.p5.loadSound(
      "assets_game/mlxScaventureHunt/Blip.mp3"
    );
    this.sound_start = this.p5.loadSound(
      "assets_game/mlxScaventureHunt/start.mp3"
    );
    this.font = this.p5.loadFont(
      "assets_game/mlxScaventureHunt/JSJindara-Bold.otf"
    );
  }

  setup() {
    this.precanvas.textFont(this.font);
    this.myVoice.setVoice(1);
  ...
  draw(p5) {
    ...
    if (this.gameMode == "INTRO") {
      ...
      this.precanvas.text(this.currentGuess, 24, 97, 280, 46);
      this.precanvas.pop();
      //timelimit
      /*
  ...
  doProcess() {
    ...
    if (ifFound && this.gameMode == "FINDING") {
      ...
      if (
        this.previousResultWord == null ||
        this.previousResultWord != oneWordRes
      ) {
        if (this.speakObject == false) {
          this.speakObject = true;
          this.currentGuess = "Please  find " + this.currentWord;
          if (mlxScaventureHuntEndSpeak) {
            this.myVoice.speak(`Please  find ${this.currentWord}`);
  ...
  speechEnded() {
    mlxScaventureHuntEndSpeak = true;
    //this.myVoice.cancel();
