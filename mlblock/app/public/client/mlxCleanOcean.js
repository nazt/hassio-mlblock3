...
class mlxCleanOceanGame extends mlxElementLocakableOutput {
  ...
  preload() {
    ...
    this.img_base = this.p5.loadImage(
      "assets_game/mlxCleanOceanGame/baselayer.png"
    );
    ...
    this.img_logo = this.p5.loadImage(
      "assets_game/mlxCleanOceanGame/whalelogo.png"
    );
    this.img_waiting = this.p5.loadImage(
      "assets_game/mlxCleanOceanGame/waiting.png"
    );
    this.s_pickup = this.p5.loadSound(
      "assets_game/mlxCleanOceanGame/pickup.mp3"
    );
    this.s_collect = this.p5.loadSound(
      "assets_game/mlxCleanOceanGame/collect.mp3"
    );
    this.s_music = this.p5.loadSound("assets_game/mlxCleanOceanGame/music.mp3");
    for (var i = 0; i < 10; i++) {
      this.trashImage[i] = this.p5.loadImage(
        "assets_game/mlxCleanOceanGame/trash" + (i + 1) + ".png"
      );
    }
  }
  ...
  getKeypoints() {
    ...
    for (var i = 0; i < this.results.length; i++) {
      if (this.results[i].pose.score >= 0.15) {
        ...
        for (
          var j = 0;
          j < this.poses[this.poses.length - 1].pose.keypoints.length;
          j++
        ) {
	...
	*/
    ...
    for (var j = this.usersOnScreen.length - 1; j >= 0; j--) {
      for (var i = 0; i < this.usersOnScreen.length; i++) {
        if (
          j != i &&
          this.usersOnScreen[j].currentpose == this.usersOnScreen[i].currentpose
        ) {
...
class mlxCleanOceanGameUser {
    ...
    if (this.currentState == "NOTRECEIVED") {
      ...
      if (this.ticklingTime > 0 && isCollide) {
        ...
        gp.rect(
          -26,
          -12,
          54 * (this.ticklingTime / this.parent.TICKLING_TIME),
          9
        );
        gp.pop();
  ...
  matchUserWithPose() {
    ...
    var indexOfMaxValue = dists.reduce(
      (iMin, x, m, arr) => (x < arr[iMin] ? m : iMin),
      0
    ); //find min value index
    if (
      dists[indexOfMaxValue] < this.parent.PIXEL_NOSE_MOVE_BETWEEN_FRAME_ALLOWED
    ) {
      //within allow bound
      ...
      var posFacespace = this.dist(
        this.p5.createVector(
          width -
            this.parent.poses[indexOfMaxValue].pose.keypoints[0].position.x,
          this.parent.poses[indexOfMaxValue].pose.keypoints[0].position.y
        ),
        this.p5.createVector(
          width -
            this.parent.poses[indexOfMaxValue].pose.keypoints[1].position.x,
          this.parent.poses[indexOfMaxValue].pose.keypoints[1].position.y
        )
      );
      if (
        this.p5.abs(posFacespace / this.facespace) <=
  ...
  dist(v1, v2) {
    return this.p5.sqrt(
      this.p5.pow(v2.x - v1.x, 2) + this.p5.pow(v2.y - v1.y, 2)
    );
  }
