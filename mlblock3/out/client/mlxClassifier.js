...
class mlxPoseNet extends mlxElementModel {
  constructor(mlx) {
    ...
    this.model.on("pose", (results) => {
      ...
      if (this.outputType == 1) {
        this.output = results[0].pose.keypoints.map((p) => [
          p.score,
          p.position.x,
          p.position.y,
        ]);
  ...
  createElementMenu() {
    ...
    this.showKeypointsMenu = menu.addCheckedMenu(
      "Show Keypoints",
      this.showKeypoints,
      (ui, value) => {
        this.hideMenu();
    ...
    this.showSkeletonMenu = menu.addCheckedMenu(
      "Show Skeleton",
      this.showSkeleton,
      (ui, value) => {
        this.hideMenu();
    ...
    this.showNamesMenu = menu.addCheckedMenu(
      "Show Names",
      this.showNames,
      (ui, value) => {
        this.hideMenu();
    ...
    this.showOverlayMenu = menu.addCheckedMenu(
      "Show Overlay",
      this.showOverlay,
      (ui, value) => {
        this.hideMenu();
  ...
  doExtraDraw(p5, that) {
    if (that.output && that.inElement) {
      ...
      if (that.inElement.flip) {
        p5.translate(
          (that.inElement.x + that.inElement.contentX) * 2 +
            that.inElement.contentW,
          0
        );
        p5.scale(-1, 1);
      } else {
      ...
      for (let j = 0; j < pose.keypoints.length; j++) {
        ...
        if (keypoint.score > 0.2) {
          ...
          if (!isOverlay) {
            ...
            ycomp = 0;
            p5.translate(
              that.inElement.x + that.inElement.contentX,
              that.inElement.y + that.inElement.contentY - ycomp
            );
            ...
            if (this.overlayImage[keypoint.part]) {
              let is =
                (eyeDist * 0.85) / this.overlayImage[keypoint.part].width;
              p5.scale(is);
              p5.translate(
              ...
              p5.scale(that.inElement.contentScale);
              p5.image(
                this.overlayImage[keypoint.part],
                keypoint.position.x / is,
                keypoint.position.y / is
              );
            }
    ...
    for (let i = 0; i < poses.length; i++) {
      ...
      for (let j = 0; j < pose.keypoints.length; j++) {
        ...
        if (keypoint.score > 0.2) {
          ...
          p5.text(
            keypoint.part,
            keypoint.position.x - p5.textWidth(keypoint.part) / 2,
            keypoint.position.y - s * 2
          );
          p5.pop();
          p5.pop();
        }
...
class mlxHandPose extends mlxElementModel {
  ...
  doExtraDraw(p5, that) {
    if (that.output && that.inElement && that.output.length > 0) {
      ...
      p5.translate(
        that.inElement.x + that.inElement.contentX,
        that.inElement.y + that.inElement.contentY
      );
      ...
      for (let i = 0; i < detections.length; i++) {
        ...
        if (that.showBoundingBox) {
          ...
          p5.rect(
            bb.topLeft[0],
            bb.topLeft[1],
            bb.bottomRight[0] - bb.topLeft[0],
            bb.bottomRight[1] - bb.topLeft[1]
          );
        }
  ...
  doExtraDraw(p5, that) {
    if (that.output && that.inElement) {
      ...
      for (let i = 0; i < objects.length; i++) {
        ...
        if (s > 30 / that.inElement.contentScale)
          s = 30 / that.inElement.contentScale;
        if (s < 5 / that.inElement.contentScale)
          s = 5 / that.inElement.contentScale;
        p5.textSize(s);
          ...
          if (this.inElement) {
            ...
            for (let i in this.extraInElement) {
              ...
              if (elx.outType === "image" && elx.category == "Filter") {
                if (elx.filter) {
                  this.inElement.replaceImage = elx.filter(
  ...
  createParticle() {
    ...
    for (let y = 0; y < this.particleH; y++) {
      for (let x = 0; x < this.particleW; x++) {
        let v = this.p5
          .createVector(x - this.particleW / 2, y - this.particleH / 2)
          .normalize();
    ...
    for (let y = 0; y < this.particleH; y++) {
      ...
      for (let x = 0; x < this.particleW; x++) {
        ...
        if (a) {
          ...
          this.particleImage.set(
            this.particles[idx].position.x,
            this.particles[idx].position.y,
            [r, g, b, a]
          );
        }
        xx += stepX;
