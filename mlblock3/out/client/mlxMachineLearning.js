...
class mlxKNNClassifier extends mlxFeatureExtractor {
  constructor(mlx) {
    ...
    this.categoryMenu2 = this.mlx.createMenu(
      this,
      105,
      this.mlx.mlxCaptionHeight + 44 + 10
    );
    this.select2 = this.categoryMenu2.addSelectMenu(
      ["F", "G", "H", "I", "J"],
      -1,
      (ui, value) => {
        if (this.inElement && this.inElement.output) {
  ...
  updateDisplayData() {
    Object.keys(this.rawData).forEach((label, i) => {
      ...
      if (index > 4) {
        this.select2.setTitle(
          index - 5,
          `${label} (${this.rawData[label].length})`
        );
      } else {
  ...
  doProcess() {
    if (this.inElement.output) {
      ...
      let isBatch =
        Array.isArray(data) && (data[0].data || Array.isArray(data[0]));
      let isTrain = isBatch
...
class mlxConfusionMatrix extends mlxElementOutput {
  ...
  onCompute() {
    if (
      ...
      this.predicted.length == this.targeted.length
    ) {
      let predictLabels = this.predicted
        .map((el) => el.label)
        .filter((el) => el != undefined);
      let targetLabels = this.targeted
        .map((el) => el.label)
        .filter((el) => el != undefined);
      if (
        predictLabels.length == targetLabels.length &&
        predictLabels.length > 0
      ) {
        let CM2 = ConfusionMatrix.default.fromLabels(
          targetLabels,
          predictLabels
        );
        this.cellWidth =
          CM2.labels.sort((a, b) => b.length - a.length)[0].length * 6;
        this.cm = CM2;
      } else {
  ...
  draw(p5) {
    ...
    if (this.cm) {
      ...
      p5.rect(
        ...
        cellWidth * colNum,
        cellHeight
      ); //external rect (predict)
      p5.rect(
        startx,
        starty + cellHeight + cellHeight,
        cellHeight,
        cellHeight * rowNum
      ); //external rect (actual)
      p5.fill(0);
      p5.strokeWeight(0);
      p5.text(
        "Predicted",
      ...
      this.cm.getMatrix().forEach((row, j) => {
        row.forEach((value, i) => {
          ...
          if (i == j) {
            //correction
            let nagativeColor =
              (value / this.cm.getPositiveCount(this.cm.labels[i])) * 204;
      ...
      this.cm.labels.forEach((label, j) => {
        ...
        this.drawTextBox(
          this.cm.getPositiveCount(label),
          beginX,
          yinx,
          sumWidth,
          cellHeight,
        ...
        //draw recall
        this.drawTextBox(
          this.cm.getTruePositiveRate(label),
          beginX + sumWidth,
          yinx,
          recallWidth,
          cellHeight,
        ...
        //draw sum-predicted
        this.drawTextBox(
          this.cm.getTruePositiveCount(label) +
            this.cm.getFalsePositiveCount(label),
          xinx,
          beginY,
          cellWidth,
          cellHeight,
