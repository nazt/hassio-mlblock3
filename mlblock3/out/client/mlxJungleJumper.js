...
class mlxJungleJumper extends mlxElementOutput {
  constructor(mlx) {
    ...
    console.log('Game Art (Free Commercial Licens) from https://jesse-m.itch.io/jungle-pack/')
  ...
  preload() {
    ...
    this.image_landing = this.p5.loadImage('/assets_game/mlxJungleJumper/landing.png')
    this.img_slide = this.p5.loadImage('/assets_game/mlxJungleJumper/slide.png')
    this.image_particle = this.p5.loadImage('/assets_game/mlxJungleJumper/particle.png')
    this.img_tile_floor = this.p5.loadImage('/assets_game/mlxJungleJumper/tilefloor.png')
    this.img_floor_start = this.p5.loadImage('/assets_game/mlxJungleJumper/tilefloor_start.png')
    this.img_floor_end = this.p5.loadImage('/assets_game/mlxJungleJumper/tilefloor_end.png')
    ...
    this.img_food = this.p5.loadImage('/assets_game/mlxJungleJumper/sandwich.png')
    this.sound_slide = this.p5.loadSound('/assets_game/mlxJungleJumper/slide.mp3')
    ...
    this.sound_readygo = this.p5.loadSound('/assets_game/mlxJungleJumper/readygo.mp3')
    this.sound_collect = this.p5.loadSound('/assets_game/mlxJungleJumper/correct.mp3')
    this.font = this.p5.loadFont('/assets_game/mlxJungleJumper/Anakotmai-Bold.ttf')
  }
  ...
  draw(p5) {
    ...
    if (this.GAMEPHASE == 'PLAY') {
      ...
      this.precanvas.line(this.canvas_w / 2, 0, this.canvas_w / 2, this.canvas_h)
      this.precanvas.pop()
    }
    //Floor
    this.floorTile.draw()
      ...
      if (
        this.spikes[i].collide(
          this.p5.createVector(
            this.character.positionXOnScreen - this.character.characterWidth / 2,
            this.character.previousPositionY - this.character.characterHeight / 2
          ),
          this.p5.createVector(this.character.characterWidth, this.character.characterHeight)
        ) &&
        this.GAMEPHASE != 'OVER'
      ) {
        //hit spike
  ...
  resetGame() {
    ...
    this.pitCount = 0
    this.spikeCount = 0
    this.obstacleGenerator = new JungleJumperObstacleGenerator(this.floorTile.tileCount + 16, this)
...
class JungleJumperCharacter {
  ...
  calculateAndDraw(floorHeight) {
    ...
    //curretn Y pos
    var charYPos = this.parent.canvas_h - this.positionY - this.characterHeight / 2 - this.floorHeight
	...
	*/
    ...
    if (this.parent.GAMEPHASE == 'OVER') {
      ...
      //running/jumping
      charYPos = this.parent.canvas_h - this.positionY - this.characterHeight / 2 - this.floorHeight
    ...
    for (var i = 0; i < this.parent.spikeCount; i++) {
      if (
        this.parent.spikes[i].collide(
          this.parent.p5.createVector(this.positionXOnScreen, this.previousPositionY),
          this.parent.p5.createVector(this.characterWidth, this.characterHeight)
        )
      ) {
        return true
...
class JungleJumperUI {
  ...
  draw() {
    ...
    if (this.parent.GAMEPHASE == 'PREGAME') {
      ...
      this.parent.precanvas.text('"C" to Start Game', width / 2, height / 2 + 67)
      //alpha with sin()
      ...
      this.parent.precanvas.image(this.parent.img_logo, width / 2, height / 2 - 50)
    }
    //----------------------------
      ...
      if (this.countdown < 0) {
        ...
        this.parent.precanvas.text('Distance ' + this.parent.p5.int(this.parent.character.totalMeter) + ' m.', 10, 30)
        this.parent.precanvas.pop()
      } else if (this.countdown >= 0) {
        //show countdown to start
        this.parent.precanvas.textAlign(this.parent.p5.CENTER, this.parent.p5.CENTER)
        this.setTextSizeStrokeWeight(100, 0, 4)
        var alpha = 255.0 * (this.countdown - this.parent.p5.int(this.countdown))
        this.parent.precanvas.stroke(0, alpha)
        this.parent.precanvas.fill(255, 0, 0, alpha)
        if (this.countdown > 1) {
          this.parent.precanvas.text(this.parent.p5.int(this.countdown), width / 2, height / 2)
        } else {
    ...
    //----------------------------
      ...
      this.parent.precanvas.text('Food x' + this.parent.foodScore + ' piece(s).', width / 2, height / 2 + 40)
      this.parent.precanvas.fill(100, 255, 100)
      this.parent.precanvas.text(
        'Total ' + this.parent.p5.int(this.parent.character.totalMeter + this.parent.foodScore * 10) + ' point.',
        width / 2,
        height / 2 + 60
      )
      ...
      this.setTextSizeStrokeWeight(12, 0, 4)
      this.parent.precanvas.fill(255, 234, 0)
      this.parent.precanvas.text('"C" to Start Game', width / 2, height / 2 + 80)
...
class JungleJumperFloorTile {
  constructor(mlxParent) {
    ...
    this.tileCount = this.parent.p5.floor(this.parent.canvas_w / this.parent.floor_w) + 2
    this.PositionFloorTile = []
    for (var i = 0; i < this.tileCount; i++) {
      this.PositionFloorTile[i] = this.parent.floor_w / 2 + this.parent.floor_w * (i + 1)
    ...
    for (var i = 0; i < this.tileCount; i++) {
      ...
      if (this.parent.pitCount > 0) {
        for (var j = 0; j < this.parent.pitCount; j++) {
          if (this.parent.pits[j].isStartPit(this.parent.TILE_LOOP_NO[i])) type = 1
          else if (this.parent.pits[j].isEndPit(this.parent.TILE_LOOP_NO[i])) type = 2
          else if (this.parent.pits[j].isPitNone(this.parent.TILE_LOOP_NO[i])) type = 3
        }
      }
      //draw floor tile
      if (type == 0)
      ...
      if (this.PositionFloorTile[i] < -(this.parent.floor_w / 2)) {
        ...
        if (this.parent.pitCount > 0 && this.parent.pits[0].nextPit(this.parent.TILE_LOOP_NO[i])) {
  ...
  collide(charPos, charBound) {
    ...
    for (var i = 0; i < this.tileCount; i++) {
      ...
      var isPit = false
      for (var j = 0; j < this.parent.pitCount; j++) {
        if (this.parent.pits[j].isPitNone(this.parent.TILE_LOOP_NO[i])) isPit = true
      }
      if (!isPit) {
        ...
        var vBound = this.parent.p5.createVector(this.parent.floor_w, this.parent.floor_h)
        //check intersection
        if (
    ...
    if (this.currentTileCount != this.parent.TILE_TOTAL) {
      ...
      if (this.parent.TILE_TOTAL >= this.nextRandom) {
        ...
        if (ob == 2) {
          ...
          this.parent.pits[this.parent.pitCount] = new JungleJumperPitData(target, this.parent)
          this.parent.pitCount++
          ...
          //food
          var target = this.parent.TILE_TOTAL + 1 // + random(0, 8);
          this.parent.food[this.parent.foodCount] = new JungleJumperFood(target, this.parent)
          this.parent.foodCount++
          this.nextRandom = target + 9 + this.parent.p5.random(0, 5)
        } else {
          //75% chance of create spike
          var target = this.parent.TILE_TOTAL + 1 // + random(0, 8);
          this.parent.spikes[this.parent.spikeCount] = new JungleJumperSpike(target, this.parent)
...
class JungleJumperFood {
  constructor(appearOnTile, mlxParent) {
    ...
    this.PositionX = this.parent.floor_w * this.parent.floorTile.tileCount - this.parent.floor_w / 2 //set start x position to synchronize with floortile
  ...
  draw() {
    if (this.appearOnTile <= this.parent.TILE_TOTAL) {
      ...
      this.parent.precanvas.image(this.parent.img_food, this.PositionX, this.PositionY, this.food_wh, this.food_wh)
      this.parent.precanvas.pop()
  ...
  collide(charPos, charBound) {
    ...
    var vPos = this.parent.p5.createVector(this.PositionX - this.food_wh / 2, this.PositionY)
...
class JungleJumperSpike {
  ...
  collide(charPos, charBound) {
    //rect(charPos.x, charPos.y, charBound.x, charBound.y);
    var vPos = this.parent.p5.createVector(this.PositionX - this.parent.spike_w / 2 + 16, 0)
    var vBound = this.parent.p5.createVector(this.parent.spike_w - 32, this.parent.spike_h - 10)
    //rect(vPos.x, vPos.y, vBound.x, vBound.y);
    //check intersection
    if (
      vPos.x < charPos.x + charBound.x &&
...
class JungleJumperParallaxBG {
  ...
  draw() {
    ...
    for (var i = 0; i < this.count; i++) {
      this.parent.precanvas.image(this.img, this.bgTile[i], 0, this.parent.canvas_w, this.parent.canvas_h)
      this.bgTile[i] -= this.speed * this.parent.MAIN_SPEED
      if (this.bgTile[i] < -this.parent.canvas_w) this.bgTile[i] += this.parent.canvas_w * this.count
    }
    this.parent.precanvas.pop()
  }
}
