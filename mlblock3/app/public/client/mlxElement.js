...
class mlxElement {
  ...
  setPosition(x, y) {
    this.x = x
    this.y = y
    if (this.div) this.div.position(this.x + this.divX, this.y + this.divY + this.mlx.mlxNavBarHeight)
  }
  ...
  isHitMenuButton(x, y) {
    if (x > this.x + 10 && x < this.x + 20 && y > this.y + 8 && y < this.y + 26) {
      return true
    }
    return false
  }
  ...
  isHitSizer(x, y) {
    ...
    if (x > this.x + this.w - 10 && x < this.x + this.w && y > this.y + this.h - 10 && y < this.y + this.h) {
      return true
    }
    return false
  }
  ...
  isHitCaptionBar(x, y) {
    if (x > this.x && x < this.x + this.w && y > this.y && y < this.y + this.mlx.mlxCaptionHeight) {
      return true
    }
    return false
  }
  ...
  draw(p5) {
    ...
    // title bar
    p5.rect(this.x, this.y, this.w, this.mlx.mlxCaptionHeight, this.mlx.mlxCornerSize, this.mlx.mlxCornerSize, 0, 0)

    if (this.resizable) {
      p5.stroke(192)
      p5.strokeWeight(2)
      p5.line(this.x + this.w - 15, this.y + this.h, this.x + this.w, this.y + this.h - 15)
      p5.line(this.x + this.w - 10, this.y + this.h, this.x + this.w, this.y + this.h - 10)
    }
    ...
    // titile line
    p5.line(this.x, this.y + this.mlx.mlxCaptionHeight, this.x + this.w, this.y + this.mlx.mlxCaptionHeight)
  ...
  drawLink(p5) {
    ...
    if (this.linkMoving) {
      ...
      p5.noFill()
      let bx = (Math.abs(this.outElement.inputLinkX - this.outputLinkX) * 2) / 3
      let by = (Math.abs(this.outElement.inputLinkY - this.outputLinkY) * 2) / 3
      p5.bezier(
        this.outputLinkX,
        this.outputLinkY,
        this.outputLinkX + bx * this.outputLinkCtrlX,
  ...
  drawOutputLink(p5) {
    ...
    p5.ellipse(this.outputLinkX, this.outputLinkY, this.mlx.mlxLinkSize, this.mlx.mlxLinkSize, 0)
  }
...
class mlxElementInput extends mlxElement {
  ...
  w2h(w) {
    return (w - this.mlx.mlxPadding * 2) / this.contentAspectRatio + this.mlx.mlxCaptionHeight + this.mlx.mlxPadding * 2
  }

  h2w(h) {
    return (h - this.mlx.mlxCaptionHeight - this.mlx.mlxPadding * 2) * this.contentAspectRatio + this.mlx.mlxPadding * 2
  }
...
class mlxElementModel extends mlxElement {
  ...
  drawInputLink(p5) {
    ...
    if (this.inElement) {
      ...
      this.polygon(this.inputLinkX, this.inputLinkY, this.mlx.mlxLinkSize - 2, 3, this.inputLinkAngle)
    }
  }
}
...
class mlxElementLocakableOutput extends mlxElementOutput {
  ...
  canvasChanged() {
    ...
    var contentHeight = this.h - this.mlx.mlxCaptionHeight - this.mlx.mlxPadding * 2
...
class mlxLabel extends mlxElementOutput {
  ...
  createElementMenu() {
    ...
    menu.addCheckedMenu('On Screen Display', this.onScreenDisplay, (ui, value) => {
    ...
    if (Array.isArray(this.results) && this.results.length > 0) {
      if (this.results[0].label) {
        if (this.results[0].confidence) {
          if (!this.confidence80 || this.results[0].confidence > 0.8) {
            this.text = `${this.results[0].label} (${(this.results[0].confidence * 100).toFixed(2)}%)`
          } else {
            this.text = ''
          }
        } else {
      ...
      if (this.results.value) {
        ...
        if (this.results.confidence)
          this.text = `${this.results.label} (${(this.results.confidence * 100).toFixed(2)}%)`
...
class mlxOscillator extends mlxElementOutput {
  constructor(mlx) {
    ...
    this.text = ''
    this.createElementMenu()
    this.label = this.mlx.createLabel(this, 'Loading...', 18, this.mlx.mlxCaptionHeight + 5, this.w - 20, 24)
    this.onScreenDisplay = false
    this.currentNote = ''
...
class mlxConsoleLog extends mlxElementOutput {
  constructor(mlx) {
    ...
    this.text = ''
    this.createElementMenu()
    this.label = this.mlx.createLabel(this, 'Ready', 18, this.mlx.mlxCaptionHeight + 5, this.w - 20, 24)
    this.onScreenDisplay = false
...
class mlxAudioInput extends mlxElementInput {
  constructor(mlx) {
    ...
    this.icon = this.mlx.createImage(this, '/assets/iconMicrophone.jpg', 1, this.mlx.mlxCaptionHeight + 1, 198, 44)
    this.label = this.mlx.createLabel(this, 'Ready', 18, this.mlx.mlxCaptionHeight + 48, this.w - 20, 24)
...
class mlxNatsSoundTensor extends mlxElementOutput {
  ...
  doProcess() {
    if (this.inElement && this.inElement.outType == 'tensor' && this.inElement.outputIsReady) {
      this.busy = true
