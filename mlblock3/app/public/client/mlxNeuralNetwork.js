...
class mlxNeuralNetworkClassifier extends mlxNeuralNetwork {
    ...
    this.addLayerButtron = this.mlx.createButton(
      ...
      () => {
        ...
        if (player) {
          ...
          if (this.customLayersInfo.length >= 2) {
            this.customLayersInfo.splice(
              this.customLayersInfo.length - 1,
              0,
              layer
            );
          } else {
          ...
          this.classifier.train(
            ...
            (epoch, logs) => {
              ...
              if (lossValue) {
                ...
                this.labelTrain.setText(
                  `${epoch} Loss: ` + this.loss.toFixed(2)
                );
            ...
            () => {
              ...
              this.labelTrain.setText(
                "Done Training! Final Loss: " + this.loss.toFixed(2)
              );
              this.trained = true;
              this.drawed = false;
              console.log(this.classifier);

              this.autoLayersInfo = this.layerToLayerInfo(
                this.classifier.neuralNetwork.model.layers
              );
  ...
  createAddUI() {
    this.categoryMenu = this.mlx.createMenu(
      this,
      10,
      this.mlx.mlxCaptionHeight + 44 + 10
    );
    this.select = this.categoryMenu.addSelectMenu(
      ["A", "B", "C", "D", "E"],
      -1,
      (ui, value) => {
        if (this.inElement && this.inElement.output) {
    ...
    this.getButton = this.mlx.createButton(
      ...
      () => {
        if (this.inElement && this.inElement.output) {
          ...
          this.select.setTitle(
            this.currentClassIndex,
            `${this.currentClass} (${this.counts[this.currentClassIndex]})`
          );
  ...
  createActivationMenu() {
    this.menuActivation = this.mlx.createMenu(
      this,
      10,
    ...
    this.submenuActivation = this.menuActivation.addSelectMenu(
      this.activationList,
      2,
      (ui, value) => {
  ...
  createLayerTypeMenu() {
    this.menuLayerType = this.mlx.createMenu(
      this,
      10,
      this.mlx.mlxCaptionHeight
    );
    ...
    this.submenuLayerType = this.menuLayerType.addSelectMenu(
      this.layerTypeList,
      2,
      (ui, value) => {
        //console.log(this.currentActiveLayer, value);
  ...
  removeLayer(layer) {
    ...
    if (layer < this.customLayersInfo.length)
      this.customLayersInfo[layer].input =
        this.customLayersInfo[layer - 1].units;
    this.updateLayersView();
  }
  ...
  mousePressed() {
    ...
    if (!this.isAutoNeuron) {
      ...
      for (let i = 0; i < this.layers.length; i++) {
        ...
        let x0 =
          (this.w - this.mlx.mlxPadding - (layer.units - 1) * 12) / 2 - 6;
        ...
        if (mY >= y && mY < y + this.layerH) {
          if (mX < 15) {
            ...
            this.submenuActivation.setValue(
              this.activationList.indexOf(activation)
            );
            ...
            this.submenuLayerType.setValue(
              this.layerTypeList.indexOf(layerType)
            );
            this.currentActiveLayer = i;
  ...
  drawNumNeuronButton(p5, y, i, isCustom, layer) {
    ...
    if (isCustom) {
      if (layer.type == "dense") {
        ...
        let text = `${
          layer.filters
        }x[${layer.kernelSize.toString()}] >[${layer.strides.toString()}]`;
  ...
  drawLayer(p5, y, num, color, activation, deletable, type, input) {
    ...
    p5.text(
      activation,
      this.x + this.w - this.mlx.mlxPadding - 20,
      this.y + y - 3
    );
    p5.fill(0);
    if (deletable)
      p5.text("x", this.x + this.w - this.mlx.mlxPadding, this.y + y - 2);
  }
  ...
  draw(p5) {
    ...
    if (this.drawLayers && this.drawLayers.length > 0) {
      ...
      for (i in this.drawLayers) {
        ...
        {
          p5.fill(0, 0, 255);
          this.drawNumNeuronButton(
            p5,
            y,
            i,
            i < this.drawLayers.length - 1,
            layer
          );
          p5.fill(0);
        }
...
class mlxNeuralNetworkRegressor extends mlxNeuralNetworkClassifier {
  ...
  addData(data, value) {
    ...
    this.labelTrain.setText(
      `Shape: [${this.dataShape.toString()}] (${this.totalCounts} added)`
    );
  }
