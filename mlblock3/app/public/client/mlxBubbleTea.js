...
class mlxBubbleTeaGame extends mlxElementLocakableOutput {
  constructor(mlx) {
    ...
    this.h = this.cv_height / 2 + this.mlx.mlxCaptionHeight + this.mlx.mlxPadding * 2 //size including frame
    this.fps = 15
  ...
  preload() {
    this.img_waiting = this.p5.loadImage('/assets_game/mlxBubbleTea/waiting.png')
    ...
    this.img_direction = this.p5.loadImage('/assets_game/mlxBubbleTea/direction.png')
    ...
    this.img_sugar = this.p5.loadImage('/assets_game/mlxBubbleTea/sugarcube.png')
    this.img_mouth_open = this.p5.loadImage('/assets_game/mlxBubbleTea/mouth_open.png')
    this.img_mouth_close = this.p5.loadImage('/assets_game/mlxBubbleTea/mouth_close.png')
    this.img_mouth_hold = this.p5.loadImage('/assets_game/mlxBubbleTea/mouth_hold.png')
    ...
    this.sound_timebeep = this.p5.loadSound('/assets_game/mlxBubbleTea/timebeep.mp3')
    this.sound_applause = this.p5.loadSound('/assets_game/mlxBubbleTea/applause.mp3')
    this.sound_burp = this.p5.loadSound('/assets_game/mlxBubbleTea/burp.mp3')
    this.sound_jingle = this.p5.loadSound('/assets_game/mlxBubbleTea/jingle.mp3')
    this.sound_powerup = this.p5.loadSound('/assets_game/mlxBubbleTea/powerup.mp3')
  }
  ...
  enterFullScreen() {
    ...
    this.canvas.resizeCanvas(this.cv_width * this.fullscreen_scale, this.cv_height * this.fullscreen_scale)
  ...
  draw(p5) {
    ...
    if (this.inElement != null && this.inElement.inElement != null && this.inElement.inElement.videoCapture != null) {
      ...
      this.precanvas.image(this.inElement.inElement.videoCapture, 0, 0, this.cv_width, this.cv_height)
      this.precanvas.pop()
      //----------------------
      //Game draw
      ...
      if (this.gameMode == 'PLAY') {
        ...
        this.precanvas.push()
        this.precanvas.imageMode(this.p5.CENTER)
        this.precanvas.image(this.img_b1, this.cv_width / 2, this.cv_height / 2)
        this.precanvas.image(this.img_direction, this.cv_width / 2, this.cv_height / 2 + 154 + 53 + 5)
        //flash logo
        this.precanvas.push()
        this.alpha_1 += (p5.deltaTime / 50.0) % 50
        this.precanvas.tint(255, p5.abs(p5.sin((p5.TWO_PI / 50.0) * this.alpha_1)) * 255)
        this.precanvas.image(this.img_logo, this.cv_width / 2, this.cv_height / 2)
        this.precanvas.pop()
        this.mouth.draw()
        this.drawTeas()
        this.precanvas.pop()
        ...
        this.precanvas.image(this.img_b2, this.cv_width / 2, this.cv_height / 2, 0)
        this.mouth.draw()
        this.drawTeas()
        this.precanvas.pop()
      ...
      this.precanvas.push()
      this.precanvas.imageMode(this.p5.CORNER)
      this.precanvas.image(this.img_waiting, 0, 0, this.cv_width, this.cv_height)
      this.precanvas.pop()
  ...
  drawGameUI() {
    ...
    this.precanvas.text('Tea Left: ' + this.teaCountdown, this.cv_width - 220, 55, 210, 50)
    ...
    if (this.timeCountdown > 0) {
      ...
      this.precanvas.text(this.p5.floor(this.timeLimit / 1000) + 1, 5 + 52 + 5, 5, 100, 48) //time text
    }
    this.precanvas.pop()
  }
  ...
  setupLevel2() {
    ...
    this.setupTeas()
    this.walls.splice(0, this.walls.length)
    this.walls[0] = new BubbleTeaGameWall(this, 147, 113, 122, 110, this.img_w2)
    this.walls[1] = new BubbleTeaGameWall(this, 615, 324, 122, 110, this.img_w2)
  }
  ...
  setupLevel4() {
    ...
    this.walls[1] = new BubbleTeaGameWall(this, 135, 352, 110, 127, this.img_w4)
    this.walls[2] = new BubbleTeaGameWall(this, 479, 169, 211, 254, this.img_w4)
  }
  ...
  calculateMouthPos() {
    ...
    if (this.inElement.inElement.flip) xpos = this.inElement.inElement.videoWidth - mouthCenter.x
    this.mouth.position.x = (xpos / this.inElement.inElement.videoWidth) * this.cv_width
    this.mouth.position.y = (mouthCenter.y / this.inElement.inElement.videoHeight) * this.cv_height
    //console.log(this.mouth.position.x + "|" + this.mouth.position.y)
    //calculate mouth pos
    var p1 = this.p5.abs(this.p5.dist(this.noseTip.x, this.noseTip.y, this.lipTop.x, this.lipTop.y))
    var p2 = this.p5.abs(this.p5.dist(this.lipBottom.x, this.lipBottom.y, this.lipTop.x, this.lipTop.y))
  ...
  doProcess() {
    ...
    if (detections != null && detections.length > 0 && detections[0].landmarks) {
...
class BubbleTeaGameWall {
  ...
  collideMouth() {
    ...
    var DeltaX =
      ...
      this.parent.p5.max(this.x, this.parent.p5.min(this.parent.mouth.position.x, this.x + this.w))
    var DeltaY =
      this.parent.mouth.position.y -
      this.parent.p5.max(this.y, this.parent.p5.min(this.parent.mouth.position.y, this.y + this.h))
    return DeltaX * DeltaX + DeltaY * DeltaY < this.parent.mouth.radius * this.parent.mouth.radius
  }
  ...
  collide() {
    ...
    for (let i = 0; i < this.parent.teas.length; i++) {
      if (!this.parent.teas[i].eaten && !this.parent.teas[i].markForDelete) {
        ...
        var max_x = this.parent.p5.max(this.x, this.parent.p5.min(this.parent.teas[i].x, this.x + this.w))
        var max_y = this.parent.p5.max(this.y, this.parent.p5.min(this.parent.teas[i].y, this.y + this.h))
        var DeltaX = this.parent.teas[i].x - max_x
        var DeltaY = this.parent.teas[i].y - max_y
        if (DeltaX * DeltaX + DeltaY * DeltaY < this.parent.teas[i].radius * this.parent.teas[i].radius) {
          //hit
          //check size
          if (this.x == max_x) {
            //left side
            ...
            //right side
            this.parent.teas[i].x = this.x + this.w + this.parent.teas[i].radius
            this.parent.teas[i].vx *= this.parent.friction
          }
          ...
          if (this.y == max_y) {
            ...
            //bottom side
            this.parent.teas[i].y = this.y + this.h + this.parent.teas[i].radius
            this.parent.teas[i].vy *= this.parent.friction
          }
        }
      }
  ...
  checkTeaInitialPos(tea) {
    //check intersec with block(circle to rect check)
    var DeltaX = tea.x - this.parent.p5.max(this.x, this.parent.p5.min(tea.x, this.x + this.w))
    var DeltaY = tea.y - this.parent.p5.max(this.y, this.parent.p5.min(tea.y, this.y + this.h))
...
class BubbleTeaGameTea {
  ...
  collide() {
    ...
    for (let i = 0; i < this.parent.teas.length; i++) {
      if (!this.others[i].eaten && !this.others[i].markForDelete && this.parent.teas[i] != this) {
...
class BubbleTeaGameMouth {
  ...
  draw() {
    ...
    if (this.comboCountdown > 0) {
      ...
      this.parent.precanvas.text(this.comboText1, this.position.x - 60, this.position.y - this.radius - 45, 120, 25)
      this.parent.precanvas.text(this.comboText2, this.position.x - 60, this.position.y - this.radius - 20, 120, 25)
      this.parent.precanvas.image(this.parent.img_mouth_hold, this.position.x, this.position.y)
    }
    if (this.parent.mouthRecover > 0) {
      ...
      this.parent.precanvas.textSize(12)
      this.parent.precanvas.textAlign(this.parent.p5.CENTER, this.parent.p5.CENTER)
      this.parent.precanvas.text('HIT WALL!', this.position.x - 60, this.position.y - this.radius - 45, 120, 25)
      this.parent.precanvas.text(
        'Recover in ' + (this.parent.p5.floor(this.parent.mouthRecover / 1000) + 1),
        this.position.x - 60,
      ...
      this.parent.precanvas.circle(this.position.x, this.position.y, this.diameter)
      //mouth close
      this.parent.precanvas.image(this.parent.img_mouth_close, this.position.x, this.position.y)
    } else {
      //draw radius
      this.parent.precanvas.fill(0, 0, 255, 60)
      this.parent.precanvas.circle(this.position.x, this.position.y, this.diameter)
      //mouth open
      this.parent.precanvas.image(this.parent.img_mouth_open, this.position.x, this.position.y)
    }
    this.parent.precanvas.pop()
  }
}
